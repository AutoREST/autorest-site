...
function Updater(options){
    ...
    this.jarName = 'rest-api-generator-with-dependencies.jar';
    this.jarFullName = __dirname +'/'+ obj.jarName;
    this.userAgent = 'lasaro-dumer';
    this.latestReleaseURL = 'https://api.github.com/repos/AutoREST/rest-api-generator/releases/latest';
    this.versionFile = __dirname + '/version.txt';
    this.config = {};
    options = options || {};
    this.config.userAgent = options.userAgent || defaultConfig.userAgent;
    this.config.binaries = options.binaries ;//|| defaultConfig.binaries;
    this.status = {};
    this.statusTarget = 0;
    this.handleLatestRelase = function (error, response, body) {
        var headers = JSON.parse(JSON.stringify(response.request.headers));
        var name = headers['X-Request-ID'];
        var binary = obj.config.binaries.get(name);
        console.log('received to: ' + name, binary);
        // console.log(binary);
